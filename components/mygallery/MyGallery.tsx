...
function MyGallery({ title, description, images, quantity }: Props) {
  ...
  return(
    <div class="w-full container mx-auto flex flex-col">
      {title && <div class="text-5xl uppercase pb-8 p-4" dangerouslySetInnerHTML={{ __html: title }}></div>}
      ...
      </ul>
      {
        quantity < images.length && (
          <button className="btn btn-primary block w-28 mt-8 mx-auto" { ...usePartialSection({props: {quantity: quantity + 3}}) }>
            Ver mais
          </button>
        )
      }
